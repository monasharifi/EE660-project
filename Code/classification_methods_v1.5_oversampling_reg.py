...
train = genfromtxt('oversampling.csv', delimiter=',') #  Autism group1to5 _Autuism_Healthy_roi.csv
train = train[1: , :]
...
print len(inds_0)
'''
indx=np.concatenate((inds_0,inds_0),axis=0)
train= train[indx,:]  
np.random.shuffle(train)

inds_0 = indices(train [:,train.shape[1]-1], lambda x: x == 0)
indx1=np.concatenate((inds_0,inds_0),axis=0)
train= train[indx1,:]
np.random.shuffle(train)

inds_0 = indices(train [:,train.shape[1]-1], lambda x: x == 0)
indx2=np.concatenate((inds_0,inds_0),axis=0)
train= train[indx2,:]
print train.shape
np.random.shuffle(train)

'''
#mRMR =[249,56,142,121,198,70,186,248,0,255,206,128,86,196,174,214,246,71,10,199,216,119,155,185,57,188,129,14,207,247,118,204,143,	215,87,133,250,218,5,252,113,184,140,251,158,122,244,1,134,97]
y_train = train[:,train.shape[1]-1]
mRMR =[249,56,142,121,198,70,186,248,0,255,206,128,86,196,174,214,246,71,10,199,216,119,155,185,57,188,129,14,207,247]
#x_train = train[:,:train.shape[1]-1] ### Comment out for non_mRMR
x_train = train[:,mRMR]
#np.savetxt("oversampling.csv", train, delimiter=",") 
...
####*********************************#####
'''
print ('dataset size before splitting ')
print x_train.shape
...
clf = linear_model.RidgeCV(alphas= [0.1, 1.0, 10.0]) # [0.1, 1.0, 10.0]
clf.fit(x_train,y_train) 
