# *******this version is for multiclass classification ******
...
# return seperate teain and test splits for both X and Y"
    ...
    label_2 = indices(y_train , lambda x: x == 2)
    label_3 = indices(y_train , lambda x: x == 3)
    print len(label_1)
    print len(label_2)
    print len(label_3)
    # ------ extrating test data split 
    test_split_ratio= split_ratio#0.2 # input of the function 
    test_range=np.int8(len(label_1)*test_split_ratio)
    test_indx_1=label_1[0:test_range]
    test_indx_2=label_2[0:test_range]
    test_indx_3=label_3[0:test_range]
    test_split_x=np.concatenate((x_train[test_indx_1,:],x_train[test_indx_2,:],x_train[test_indx_3,:]),axis=0)
    test_split_y=np.concatenate((y_train[test_indx_1],y_train[test_indx_2],y_train[test_indx_3]),axis=0)
    
    # ------ extrating train data split 
    train_indx_1=label_1[test_range:]
    train_indx_2=label_2[test_range:]
    train_indx_3=label_3[test_range:]
    train_split_x=np.concatenate((x_train[train_indx_1,:],x_train[train_indx_2,:],x_train[train_indx_3,:]),axis=0)
    train_split_y=np.concatenate((y_train[train_indx_1],y_train[train_indx_2],y_train[train_indx_3]),axis=0)
...
train = genfromtxt('Autism group1to5 _multiclass_ROI.csv', delimiter=',') # no activation 
train = train[1: , :]
print train.shape
train= unique(train)
print train.shape
...
inds_2 = indices(train [:,train.shape[1]-1], lambda x: x == 2)
inds_3 = indices(train [:,train.shape[1]-1], lambda x: x == 3)
print len(inds_1)
print len(inds_2)
print len(inds_3)
D=np.minimum(len(inds_1),len(inds_2))
indx=np.concatenate((inds_1[0:D],inds_2[0:D],inds_3[0:D]),axis=0)
print indx
...
#np.savetxt("preprocessed_data_multiclass.csv", preprocessed_data, delimiter=",") 
#x_train = train[:,[249,71,255,143,248,199,121,57,215,0,142,70,247,10,86,251,214,56,133,14]]

############## Standarize the features
scaler=preprocessing.StandardScaler().fit(x_train)
x_train= scaler.transform (x_train)
#final_data = np.column_stack((x_train,y_train))
#np.savetxt("preprocessed_data_ROI_sub.csv", final_data, delimiter=",") 
